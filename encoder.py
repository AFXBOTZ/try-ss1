...
_chat_id = os.getenv("CHAT_ID", "0").strip()

def emergency_alert(msg, token, chat):
    if token and chat != "0" and chat != "none":
        try:
            url = f"https://api.telegram.org/bot{token}/sendMessage"
            payload = {"chat_id": chat, "text": f"🚨 **WORKER CRASH:**\n\n`{msg[-3000:]}`", "parse_mode": "Markdown"}
...
try:
    ...
    # BYPASS GITHUB SECRETS: Extracting keys from payload directly
    API_ID = int(USER_SETTINGS.get('__api_id', os.getenv("API_ID", "0")))
    API_HASH = USER_SETTINGS.get('__api_hash', os.getenv("API_HASH", ""))
    BOT_TOKEN = USER_SETTINGS.get('__bot_token', os.getenv("BOT_TOKEN", ""))
    
    # Update global placeholders for emergency alert
    _bot_token = BOT_TOKEN
    _chat_id = str(CHAT_ID)

    if API_ID == 0 or not API_HASH or not BOT_TOKEN:
        raise ValueError("GitHub Worker Error: Missing API_ID, API_HASH, or BOT_TOKEN. Check payload from bot.")
    ...
    emergency_alert(traceback.format_exc(), _bot_token, _chat_id)        if len(parts) > 3: RESOLUTION = parts[3]
        if len(parts) > 4:
            try: 
                raw_set = parts[4]
                try:
    ...
    async def process_all():
        ...
        try:
            ...
            if TASK_TYPE == "hardsub":
                ...
                # Safe Filter using absolute strings
                sub_filter = f"subtitles='{sub_path}':fontsdir='{fonts_dir}'" if sub_path else ""
    ...
    emergency_alert(traceback.format_exc())        LOGO_ID = "none"
