...
# Agar script import hone me ya run hone me fail hoti hai, toh direct Telegram par error aayega!
def emergency_alert(msg):
    bot_token = os.getenv("BOT_TOKEN", "").strip()
    chat_id_str = os.getenv("CHAT_ID", "0").strip()
    if bot_token and chat_id_str != "0" and chat_id_str != "none":
        try:
            url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
            payload = {
                "chat_id": chat_id_str, 
                "text": f"🚨 **GITHUB WORKER CRASHED:**\n\n`{msg[-2000:]}`", 
                "parse_mode": "Markdown"
            }
...
try:
    ...
    if API_ID == 0 or not API_HASH or not BOT_TOKEN:
        raise ValueError("🚨 GitHub Secrets (API_ID, API_HASH, BOT_TOKEN) are missing! Please set them in your GitHub Repo Settings -> Secrets and variables -> Actions.")
