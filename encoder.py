...
if ":::" in raw_dump:
    ...
    if len(parts) > 4:
        try: 
            # 100% BULLETPROOF STRING -> DICT Parser
            settings_str = parts[4]
            USER_SETTINGS = ast.literal_eval(settings_str)
...
async def process_all():
    ...
    try:
        ...
        if TASK_TYPE == "hardsub":
            # Direct path with no extra quotes, safe because renaming removed all brackets/spaces
...
if __name__ == "__main__":
    ...
    loop.run_until_complete(process_all())    if len(parts) > 2: STATUS_MSG_ID = parts[2]
    if len(parts) > 3: RESOLUTION = parts[3]
    if len(parts) > 4:
        try: 
            b64_str = parts[4]
...
async def process_all():
    ...
    try:
        ...
        orig_vid = await app.download_media(VIDEO_ID, progress=progress_bar, progress_args=(app, msg_id, "📥 Downloading Video"))
        if not orig_vid:
            await app.edit_message_text(CHAT_ID, msg_id, "❌ **Error:** Failed to download video! Reference Expired.")
            await app.stop()
            return
        ...
        if TASK_TYPE == "hardsub":
            # 100% Safe Filename inside filter. No quotes needed because 'safe_sub.ext' has no special chars
            sub_filter = f"subtitles={sub_path}:fontsdir=fonts" if sub_path else ""
        ...
        try: await app.edit_message_text(CHAT_ID, msg_id, f"❌ **Fatal Worker Crash:**\n\n`{err[-1000:]}`")
        except: pass
    finally:
        await app.stop()
